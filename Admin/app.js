// app.js — versão limpa e defensiva (corrigida: sem await fora de async)
(function(){
  'use strict';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      ...
      // expected ids (no abort — only warnings)
      ...
      // elements (may be null — code protects uses)
        ...
        for(const i of filtered.slice().reverse()){
          ...
          try{
            ...
            if(imgs && imgs.length > 0){
              const d = imgs[0];
              let blobData;
              if(d.thumb){ blobData = new Blob([d.thumb], { type: d.thumbType || d.type || 'image/jpeg' }) }
              else { blobData = new Blob([d.blob], { type: d.type || 'image/jpeg' }) }
      ...
      function getFormData(){
        ...
        inputs.forEach(k=>{
          const el = $id(k);
          obj[k] = el && el.value ? el.value.trim() : '';
        });
        return obj;
      }
      ...
      if(tbody) tbody.addEventListener('click', async e=>{
        ...
        if(action==='view'){
          ...
          try{
            ...
            if(!imgs || imgs.length===0){ if(modalContent) modalContent.innerHTML = '<div class="small muted">Sem imagens para este instrumento.</div>'; return; }
            const out = document.createElement('div'); out.className = 'gallery';
            imgs.forEach(r=>{
              const blob = r.thumb ? new Blob([r.thumb], {type: r.thumbType || r.type || 'image/jpeg'}) : new Blob([r.blob], {type: r.type || 'image/jpeg'});
              const url = URL.createObjectURL(blob);
              const img = document.createElement('img'); img.src = url; img.style.maxWidth = '220px'; img.style.borderRadius='8px'; img.alt = r.name;
              out.appendChild(img);
            });
      ...
      // export JSON (SUBSTITUIU CSV)
      if(btnExport) btnExport.addEventListener('click', ()=> {
        if(instruments.length===0){ alert('Nenhum instrumento para exportar.'); return; }
        // Garantimos que todos os campos estejam sanitizados antes de exportar
      ...
      if(fileImport) fileImport.addEventListener('change', ()=>{
        ...
        const reader = new FileReader(); reader.onload = ()=>{
          try{ const data = JSON.parse(reader.result);
            if(!Array.isArray(data)){ alert('Arquivo inválido: JSON deve ser um array de instrumentos'); return; }
            const existingIds = new Set(instruments.map(i=>i.id));
            let added=0;
            // Sanitiza os itens importados
      ...
      async function exportAllImagesZip(){
        try{
          ...
          if(!rows || rows.length===0){ alert('Nenhuma imagem encontrada no IndexedDB.'); return; }
          const zip = new JSZip();
          rows.forEach(r=>{
        ...
        }catch(err){ console.error(err); alert('Erro ao exportar imagens: '+(err && err.message?err.message:err)); }
      }

      if(controlsEl){
        const zipBtn = document.createElement('button'); zipBtn.className='btn ghost'; zipBtn.textContent='Exportar Imagens (ZIP)'; zipBtn.style.marginLeft='6px';
        zipBtn.addEventListener('click', exportAllImagesZip);
        controlsEl.appendChild(zipBtn);
      }

      // export to project (File System Access API) — kept, but optional; omitted here for brevity
      // you can re-add the module if needed; it was the same logic as previous versions.

      // initial render (call without top-level await)
      ...
      console.error('Fatal error initializing app.js', ex);
      alert('Erro ao iniciar o admin — veja o console para detalhes: ' + (ex && ex.message ? ex.message : ex));
    }
  });
